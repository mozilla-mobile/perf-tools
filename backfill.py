...
def prase_args():
    ...
    parser.add_argument("-ed", "--enddate", type=lambda date: datetime.strptime(date, DATETIME_FORMAT),
                        help="end date to backfill until.If empty, default will be the current date")
    parser.add_argument("-rp", "--repository_path",
    ...
    parser.add_argument("-cl", "--cleanup", choices=[True, False], default=False,
...
def run_measure_start_up_script(path_to_measure_start_up_script, durations_output_path):
    output = subprocess.run([path_to_measure_start_up_script, "nightly", "cold_view_nav_start", durations_output_path],
